            ...
            {reporter && (
              <div className="border-2 border-black rounded-xl p-6 bg-gray-50 mt-10">
                <div className="flex items-center gap-4 mb-5 border-b-2 border-black pb-4">
                  {reporter.avatar_url ? (
                    <img src={reporter.avatar_url} alt={reporter.display_name || 'Profile'}
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  {reporter.phone_number && (
                    <a href={`tel:${reporter.phone_number}`}
                      className="flex-1 flex items-center justify-center gap-2 bg-wagashi-kinako border-2 border-black px-4 py-4 rounded-xl font-black shadow-paper-sm hover:shadow-paper transition-all text-black">
                      <Phone size={20} /><span>โทรติดต่อ</span>
                    </a>
                  )}
                  {reporter.line_id && (
                    <a href={`https://line.me/ti/p/~${reporter.line_id}`} target="_blank" rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-2 bg-[#00B900] text-white border-2 border-black px-4 py-4 rounded-xl font-black shadow-paper-sm hover:shadow-paper transition-all">
                      <MessageCircle size={20} /><span>แอดไลน์</span>
                    </a>
                  )}
                  {reporter.contact_link && (
                    <a href={reporter.contact_link.startsWith('http') ? reporter.contact_link : `https://${reporter.contact_link}`}
                      target="_blank" rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-2 bg-black text-white border-2 border-black px-4 py-4 rounded-xl font-black shadow-paper-sm hover:shadow-paper transition-all">
