...
export default function NewPetPage() {
  ...
  // ── Upload images to Supabase Storage (Optimized with Promise.all) ──
  const uploadImages = async (petId: string, userId: string) => {
    const uploadPromises = images.map(async (file, i) => {
      ...
      return { storage_url: publicUrl, is_primary: i === primaryIndex }
    })

    const results = await Promise.all(uploadPromises)
    // กรองค่า null ที่เกิดจาก error ออก
    const urls = results.filter((result): result is { storage_url: string; is_primary: boolean } => result !== null)
